...
def _verify_hmac_signature(body: bytes) -> bool:
    # HMAC is optional and only meaningful for trusted server-side callers.
    # Browser clients cannot safely keep a signing secret, so unsigned requests
    # remain valid. If a caller provides a signature, we verify it.
...
def signup_endpoint():
    ...
    if len(password) < PASSWORD_MIN_LEN or password.lower() in COMMON_PASSWORDS:
        return api_error("weak_password", "Password does not meet minimum requirements.", 400)
            ...
            if USE_POSTGRES_AUTH:
                ...
                try:
                    ...
                    if getattr(e, "pgcode", None) == "23505":
                        return api_error("user_exists", "A user with that X link already exists.", 409)
...
def detect_topic(text: str) -> str:
    ...
    # Follower / growth / milestone tweets (hit 2k, 10k fam, etc.)
    if t:
        ...
        if (
            ...
            if any(
                ...
                return "milestone"
            if _re_topic.search(r"\b\d{2,}\s*(k|m)?\s*(followers|follower|subs|subscribers|fam|holders)\b", t):
                return "milestone"

    # Support / sad / personal low moments
    if any(k in t for k in ("sad", "depressed", "heartbroken", "grief", "lost my", "rip", "rest in peace", "passed away", "can't do this", "i'm tired", "im tired", "burnt out", "anxious", "panic")):
        return "support"

    # Loss / liquidation / rekt
    if any(k in t for k in ("liquidated", "liq'd", "liqd", "got liq", "rekt", "blew up", "margin call", "stopped out", "down bad", "lost it all", "wiped")):
        return "loss"

    # Scam / hack / rug / exploit
    if any(k in t for k in ("scam", "rug", "rugged", "phishing", "hacked", "drained", "exploit", "malware", "fake link", "wallet drained", "approval scam")):
        return "scam"

    # Builder / shipping / product progress
    if any(k in t for k in ("shipped", "shipping", "built", "building", "deploy", "deployed", "release", "launched", "v1", "v2", "devlog", "open sourced", "open-sourced", "commit", "pull request", "frontend", "backend")):
        return "builder"

    # DeFi yields / LP / TVL / vaults
    if any(k in t for k in ("tvl", "apy", "apr", "yield", "vault", "lp", "liquidity pool", "farm", "emissions", "fees", "rebalance", "pool", "curve", "aave", "compound", "lending", "borrow")):
        return "defi"

    # Perps / derivatives
    if any(k in t for k in ("perps", "perp", "funding", "open interest", "oi", "leverage", "longs", "shorts", "liq", "liquidation", "basis", "mark price")):
        return "perps"

    # Greetings
    if any(k in t for k in ("gm ", "gn ", "good morning", "good night", "gmgm", "gm!", "gn!")):
        return "greeting"

    # Airdrops / giveaway / mints
    if any(k in t for k in ("airdrop", "whitelist", "wl spot", "mint is live", "claim", "claimed", "allocation")):
        return "giveaway"

    # Charts / trading posts
...
REACTION_MATRIX: dict[str, dict] = {
    ...
    # Airdrops / giveaways: usually congrats + light hype
    "giveaway": {
        "bullish": ["congrats", "agree_plus", "banter"],
        "neutral": ["congrats", "agree_plus"],
        "bearish": ["question"],
    },
    # Follower / growth milestones (2k, 10k, 20k, etc.)
    "milestone": {
    ...
    "support": {
        "bullish": ["support", "question"],
        "neutral": ["support", "question"],
        "bearish": ["support", "question"],
    },
    # Loss / liquidation
    "loss": {
        "bullish": ["support", "question"],
...
class OfflineCommentGenerator:
    ...
    P(f"Does {focus_slot} hold when volatility spikes?"),
        ...
        if topic == "chart":
            ...
            buckets["support"] = [
                P(f"Sorry you are going through {focus_slot}, genuinely"),
                P(f"Hope you get some peace around {focus_slot} soon"),
                P(f"Sending strength — {focus_slot} is heavy"),
                P(f"Take it one step at a time with {focus_slot}"),
            ]
            buckets["support_q"] = [
                P(f"Anything we can do to help with {focus_slot}?"),
                P(f"Do you want advice or just space on {focus_slot}?"),
                P(f"Hope you have someone to talk to about {focus_slot}"),
            ]
        elif topic == "loss":
            buckets["loss"] = [
                P(f"That is rough — {focus_slot} wipes happen fast"),
                P(f"Sorry bro, {focus_slot} is a brutal lesson"),
                P(f"Respect for sharing {focus_slot}, most people hide it"),
                P(f"Hope you recover from {focus_slot}, one trade at a time"),
            ]
            buckets["loss_risk"] = [
                P(f"After {focus_slot}, size down and breathe"),
                P(f"Risk management > revenge trading after {focus_slot}"),
                P(f"{focus_slot} hurts but you can reset clean"),
            ]
        elif topic == "scam":
            buckets["scam"] = [
                P(f"Good looking out — {focus_slot} scams are everywhere"),
                P(f"Appreciate the warning on {focus_slot}, saving people money"),
                P(f"Anyone seeing {focus_slot}, double-check links and approvals"),
                P(f"Wild how constant {focus_slot} phishing is lately"),
            ]
            buckets["scam_action"] = [
                P(f"Revoke approvals if {focus_slot} touched your wallet"),
                P(f"Report + mute whoever pushes {focus_slot} links"),
                P(f"Bookmark official domains to avoid {focus_slot} traps"),
            ]
        elif topic == "builder":
            buckets["builder"] = [
                P(f"Shipping {focus_slot} quietly is the real flex"),
                P(f"Love the builder energy on {focus_slot}"),
                P(f"{focus_slot} looks clean — nice execution"),
                P(f"Keep cooking on {focus_slot}, this is useful"),
            ]
            buckets["builder_q"] = [
                P(f"Any docs or demo link for {focus_slot}?"),
                P(f"What stack are you using for {focus_slot}?"),
                P(f"Any lessons learned building {focus_slot}?"),
            ]
        elif topic == "defi":
            buckets["defi"] = [
                P(f"{focus_slot} is interesting — mechanics > APY banners"),
                P(f"Where does {focus_slot} yield actually come from?"),
                P(f"Fees + emissions decide {focus_slot}, not vibes"),
                P(f"TVL is nice but {focus_slot} risk is the real story"),
            ]
            buckets["defi_risk"] = [
                P(f"Smart contract + liquidity risk matter for {focus_slot}"),
                P(f"Watch unlocks and incentives around {focus_slot}"),
                P(f"{focus_slot} looks good if exit liquidity stays real"),
            ]
        elif topic == "perps":
            buckets["perps"] = [
                P(f"Funding/OI around {focus_slot} tells the real story"),
                P(f"{focus_slot} looks like positioning, not just narrative"),
                P(f"If funding flips, {focus_slot} mood changes fast"),
                P(f"Perps can punish {focus_slot} when everyone leans same way"),
            ]
            buckets["perps_risk"] = [
                P(f"Size small until {focus_slot} perps calm down"),
                P(f"Liquidations around {focus_slot} can cascade quick"),
                P(f"Spot + perps alignment matters for {focus_slot}"),
            ]

        elif topic in ("announcement", "update", "milestone"):
            ...
            if topic == "milestone":
                buckets["milestone"] = [
                    P(f"Big congrats on {focus_slot}, that kind of growth is earned"),
                    P(f"Nice to see {focus_slot} compounding instead of stalling"),
                    P(f"Milestones like {focus_slot} hit different when it is organic"),
                    P(f"Crazy to think how far {focus_slot} has come already"),
                ]
        elif topic == "thread":
            buckets["thread"] = [
    ...
    def _english_candidate(self, text: str, ctx: Dict[str, Any]) -> Optional[str]:
        ...
        if random.random() < 0.7:
            ...
            # Prefer topic-specific buckets when present (more human / less generic)
            preferred_keys = []
            if topic == "support":
                preferred_keys = [k for k in ("support","support_q") if k in buckets]
            elif topic == "loss":
                preferred_keys = [k for k in ("loss","loss_risk") if k in buckets]
            elif topic == "scam":
                preferred_keys = [k for k in ("scam","scam_action") if k in buckets]
            elif topic == "builder":
                preferred_keys = [k for k in ("builder","builder_q") if k in buckets]
            elif topic == "defi":
                preferred_keys = [k for k in ("defi","defi_risk") if k in buckets]
            elif topic == "perps":
                preferred_keys = [k for k in ("perps","perps_risk") if k in buckets]
            elif topic == "milestone":
                preferred_keys = [k for k in ("milestone",) if k in buckets]

            if preferred_keys and random.random() < 0.75:
                kind = random.choice(preferred_keys)
            else:
    ...
    def generate_two(
        ...
        # Final safety: if we still have fewer than 2 suggestions here,
        # let higher-level logic decide how to backfill (Groq / offline fallback).
        # We only enforce a mild "not identical" constraint.
        if len(out) == 2 and _pair_too_similar(out[0]["text"], out[1]["text"]):
            # Drop the second when it's basically a duplicate; caller can refill.
            out = [out[0]]
...
# ---------------------------------------------------------------------------
# Provider output hardening (avoid empty/bad-format replies from Gemini/etc.)
# ---------------------------------------------------------------------------

_AI_BAD_PHRASES = [
    "as an ai", "i can't", "i cannot", "i’m an ai", "i am an ai",
    "here are", "below are", "i will", "sure!", "certainly",
    "i don't have access", "i do not have access", "i can't browse",
]
_BAD_META_PHRASES = [
    "comment 1", "comment 2", "reply 1", "reply 2", "option 1", "option 2",
...
def _split_to_lines(text: str) -> list[str]:
    ...
    # If the model returned a single paragraph with separators, split lightly.
    if len(lines) < 2:
        parts = re.split(r"\s*(?:\|\||\|\s|\s/\s|\s—\s|\s-\s)\s*", raw)
        parts = [p.strip() for p in parts if p.strip()]
        if len(parts) >= 2:
            lines = parts
    return lines

def _looks_like_comment(line: str, tweet_text: str = "") -> bool:
    t = (line or "").strip()
    if not t:
        return False
    low = t.lower()

    # Block meta / instruction-y outputs
    if any(p in low for p in _AI_BAD_PHRASES):
        return False
    if any(p in low for p in _BAD_META_PHRASES):
        return False
    ...
    # Avoid copying tweet verbatim
    if tweet_text:
        tw = (tweet_text or "").strip().lower()
        if tw and low == tw:
            return False

    # Avoid multiple sentences / essays
    # (Allow 2 short sentences max)
    if t.count(".") + t.count("!") + t.count("?") > 3:
        return False

    return True

def _postprocess_candidates(items: Any, tweet_text: str = "") -> list[str]:
    out: list[str] = []
    if not items:
        return out

    # Accept list/tuple or a single string
    ...
    for it in raw_list:
        ...
        if not isinstance(it, str):
            it = str(it)
        for line in _split_to_lines(it):
            line = _strip_wrappers(line)
            if not _looks_like_comment(line, tweet_text=tweet_text):
                continue
            out.append(line)
...
def _ensure_pair_via_groq(tweet_text: str, author: Optional[str], url: str = "") -> list[str]:
    """
    Hard fallback: force Groq to output exactly two natural replies.
    Used when other providers return empty / invalid format.
    """
    if not groq_enabled():
        return []
    try:
        # Keep prompt tight so it succeeds even under provider issues
        sys_msg = (
            "You write short, human Twitter replies. Output ONLY two replies, "
            "each on its own line. No numbering, no quotes, no extra text."
        )
        user_msg = (
            f"Tweet: {tweet_text}\n"
            + (f"Author: {author}\n" if author else "")
            + (f"Context URL: {url}\n" if url else "")
            + "Write two replies now."
        )
        resp = groq_chat_limited(
            messages=[
        ...
        text = (resp.choices[0].message.content or "").strip() if resp else ""
        return _postprocess_candidates(text, tweet_text=tweet_text)
    except Exception as e:  # noqa: BLE001
        logger.warning("_ensure_pair_via_groq failed: %s", e)
        return []
...
def offline_two_comments(text: str, author: Optional[str]) -> list[str]:
    ...
    # Let callers decide how to backfill if they truly need 2 comments.
    return result[:2]
...
def safe_offline_two_comments(
    ...
    This is only used when Groq or other LLM providers cannot return
    usable comments. It intentionally avoids injecting generic rescue
    lines so that higher-level logic can decide how to handle failure.
    """
    out: list[str] = []

    # 1) Try the pluggable offline_two_comments() hook if present.
    fn = globals().get("offline_two_comments")
    if callable(fn):
        try:
            out = [t for t in (fn(tweet_text, author) or []) if t]
        except Exception as e:  # noqa: BLE001
            logger.warning("offline_two_comments failed inside safe_offline_two_comments: %s", e)

    # 2) Fallback to the built-in OfflineCommentGenerator.
    if not out:
        try:
            ...
            logger.warning("OfflineCommentGenerator.generate_two failed: %s", e)
            out = []

    # 3) Post-process + de-duplicate.
    cleaned: list[str] = []
    for t in out:
        t = postprocess_comment(t, "offline")
        if t:
            cleaned.append(t)
...
def generate_two_comments_offline(tweet_text: str, author: Optional[str], url: str = "") -> list[str]:
    ...
    if not out:
        out = _rescue_two(tweet_text, _get_target_lang())[:2]
    elif len(out) == 1:
        second = next((x for x in _rescue_two(tweet_text, _get_target_lang()) if x and x != out[0]), None)
        out = [out[0], (second or out[0])]
...
# ------------------------------------------------------------------------------
def _len_ok_for_lang(text: str, lang_code: str | None) -> bool:
    t = (text or "").strip()
    if not t:
        return False
    lc = (lang_code or _get_target_lang() or "en").strip().lower()
    script = (script_from_lang_code(lc) or "latn").strip().lower()

    # CJK: use character count (spaces removed)
    if script in ("zh", "ja"):
        n = len(re.sub(r"\s+", "", t))
        return 8 <= n <= 80

    toks = tokenize_by_script(t, script=script)
    if not toks:
        return False

    # English: 6-18 words
    if lc.startswith("en") and script == "latn":
        return 6 <= len(toks) <= 18

    # Other scripts: allow a slightly wider range
    return 4 <= len(toks) <= 24


def _filter_candidates_len(cands: list[str], lang_code: str | None) -> list[str]:
    out: list[str] = []
    for c in cands or []:
        if _len_ok_for_lang(c, lang_code):
            out.append(c)
    return out

def parse_two_comments_flex(raw_text: str) -> list[str]:
    """Parse provider output into exactly two candidate strings.
    ...
    - two lines
    - delimiter-separated
    Script-aware: don't rely on whitespace word counts for CJK.
    """
    raw_text = (raw_text or "").strip()
    if not raw_text:
        return []

    lang_code = (_get_target_lang() or _detect_lang_light(raw_text) or "en").strip().lower()

    # 1) JSON array (or dict wrapping an array)
    try:
        ...
        if isinstance(data, list):
            ...
            out = _filter_candidates_len(out, lang_code)
            if len(out) >= 2:
                return out[:2]
    except Exception:
        pass
    ...
    if len(quoted) >= 2:
        out = [q.strip() for q in quoted[:4] if q.strip()]
        out = _filter_candidates_len(out, lang_code)
        if len(out) >= 2:
            return out[:2]
    ...
    # 3) Bullets / numbering
    parts = re.split(r"(?:^|\n)\s*(?:\(?\d+\)?[\).\:-]|[-•*])\s*", raw_text)
    parts = [_strip_wrappers(p) for p in parts if p and not p.isspace()]
    parts = [p for p in parts if p]
    parts = _filter_candidates_len(parts, lang_code)
    if len(parts) >= 2:
        return parts[:2]

    # 4) Lines
    lines2 = [_strip_wrappers(ln) for ln in raw_text.splitlines() if ln.strip()]
    lines2 = [ln for ln in lines2 if ln]
    lines2 = _filter_candidates_len(lines2, lang_code)
    if len(lines2) >= 2:
        return lines2[:2]

    # 5) Delimiters
    m2 = re.split(r"\s*[;|/\\]+\s*", raw_text)
    m2 = [_strip_wrappers(x) for x in m2 if x and x.strip()]
    m2 = [x for x in m2 if x]
    m2 = _filter_candidates_len(m2, lang_code)
    if len(m2) >= 2:
        return m2[:2]
    ...
    for _ in range(PRO_KOL_REWRITE_MAX_TRIES):
        for provider in _pick_rewrite_provider_order():
            try:
                ...
                cand = _filter_candidates_len(cand, _get_target_lang())
...
def comment_stream_endpoint():
    ...
    if getattr(req, "url", None):
        raw_urls.append((req.url or "").strip())
    if getattr(req, "urls", None):
        raw_urls.extend([str(u or "").strip() for u in (req.urls or []) if str(u or "").strip()])
    ...
    try:
        ...
        urls, skipped_duplicates = dedupe_urls_by_tweet_id(cleaned)
    except CrownTALKError as e:
        return api_error(e.code, str(e), 400)
    except Exception:
    ...
    def emit_status(stage: str, index: int, total: int, done: int, url: str, *, cancelled: bool = False):
        return sse_event(
            "status",
            {
                "type": "status",
    ...
    def stream():
        ...
        def cancelled_now(index: int, current_url: str):
            if not _is_run_cancelled(run_id):
                return False
            yield emit_status("cancelled", index, total, done_count, current_url, cancelled=True)
            yield sse_event(
                "done",
                {
                    "type": "done",
        ...
        def process_one(url: str, index: int):
            nonlocal done_count, ok_count, results, failed
            ...
            def fail(code: str, message: str):
                ...
                failed.append({"url": url, "reason": message, "code": code, "status": "error"})
                logger.warning("comment/stream failed code=%s url=%s msg=%s", code, url, message)
                yield sse_event("error", {"code": code, "message": message, "url": url})
                yield sse_event("result", {"type": "result", "item": err_item})
                done_count += 1
                yield emit_status("finalizing", index, total, done_count, url)
                yield emit_status("done", index, total, done_count, url)

            if (yield from cancelled_now(index, url)):
                return True

            yield emit_status("queued", index, total, done_count, url)
            yield emit_status("fetching", index, total, done_count, url)
            ...
            yield emit_status("generating", index, total, done_count, url)
            handle = t.handle or _extract_handle_from_url(url)
            input_url = url
            ...
            try:
                ...
                if not comments:
                    fallback_target = "en" if target == "en" else target
                    comments = generate_two_comments_with_providers(
                        ...
                        out_lang_tag=fallback_target,
                        include_alternates=include_alts,
                    )

                yield emit_status("polishing", index, total, done_count, url)
                ...
                try:
                    ...
                    if isinstance(research_ctx, dict):
                        for proj_item in (research_ctx.get("projects") or []):
                            h = (proj_item.get("handle") or "").strip()
                            if h and h not in project_handles:
                                project_handles.append(h)
                except Exception:
                    used_research = False
                ...
                yield sse_event("result", {"type": "result", "item": result_item})
                done_count += 1
                yield emit_status("finalizing", index, total, done_count, input_url or url)
                yield emit_status("done", index, total, done_count, input_url or url)
        ...
        try:
            ...
            for index, url in enumerate(urls, start=1):
                if (yield from process_one(url, index)):
                    return

            snapshot_request = dict(payload or {})
            snapshot_request["urls"] = list(urls)
            snapshot_request["preset"] = preset
            snapshot_request["output_language"] = output_language
            snapshot_request["fast"] = quality_mode == "fast"
            snapshot_request["quality_mode"] = quality_mode
            snapshot_request["mode"] = "stream"
            snapshot_response = {"results": results, "failed": failed}
            try:
